...
const GeneratePage = () => {
  ...
  const { data: deckStatus, isLoading: isPolling } = useDeckStatus(
    deckName,
    shouldStartPolling
  );
  ...
  useEffect(() => {
    ...
    console.log("DeckStatus changed:", deckStatus);

    if (deckStatus.status === "complete" && deckStatus.data) {
      console.log("Status is complete, navigating...");
      setIsModalOpen(false);
      setTimeout(() => {
        window.scrollTo(0, 0); // 스크롤을 맨 위로
  ...
  return (
    <>
      <Helmet>
        <title>Duel | Generate Flashcard</title>
        <meta
          name="description"
          content="Upload your video or audio to generate AI-powered flashcards."
        />
        <meta property="og:title" content="Duel | Generate Flashcard" />
        <meta
          property="og:description"
          content="Upload your video or audio to generate AI-powered flashcards."
        />
        <link rel="canonical" href="https://duel.work/generate" />
      </Helmet>
